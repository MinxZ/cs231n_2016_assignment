...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #############################################################################
    # Get the mean and variance of every feature:
    ...
    cache = (mean, var, x_hat, x, gamma, eps)
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  mean, var, x_hat, x, gamma, eps = cache
  N = x.shape[0]
  '''
  dx_1 = gamma * dout

  # a.(x - mean) * b.(var + eps)**-0.5
  ...
  dmean = np.sum(dx_hat * -1/np.sqrt(var + eps) + dvar * -2 * (x - mean) / N, axis = 0)
...
def batchnorm_backward_alt(dout, cache):
  ...
  #reference:http://ryansun.logdown.com/posts/524577
  dx_hat = gamma * dout
  dx = \
  (N*dx_hat - \
  np.sum(dx_hat*x_hat, axis = 0 )* x_hat - \
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    ###########################################################################
    # drop .75 means only .25 can be keepen
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_out):
      for j in range(W_out):
          ...
          x_masked_max = \
          np.max(x_masked, axis = (2,3))
          temp_binary_mask = (x_masked == (x_masked_max)[:,:,None,None])
