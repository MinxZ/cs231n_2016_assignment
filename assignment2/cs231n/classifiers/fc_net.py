...
#        weight_scale = 1/np.sqrt(dims[i])
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
        if self.use_batchnorm:
            out, cache_layer[i] = affine_batnom_relu_forward(out,
                                                             self.params['W%d' % i],
                                                             self.params['b%d' % i],
            ...
            out, cache_layer[i] = affine_relu_forward(out,
                                                             self.params['W%d' % i],
                                                             self.params['b%d' % i])
        if self.use_dropout:
    ...
    # Forward into last layer (affine -)
    ...
    for i in reversed(xrange(1, self.num_layers)):
      # r = cache_layer[i + 1]
      if self.use_dropout:
          dx[i + 1] = dropout_backward(dx[i + 1], cache_drop[i])
      if self.use_batchnorm:
          dx[i], grads['W%d' % i], grads['b%d' % i], \
          grads['gamma%d' % i], grads['beta%d' % i]= \
          affine_batnom_relu_backward(dx[i + 1], cache_layer[i])
      else:
          dx[i], grads['W%d' % i], grads['b%d' % i]= \
